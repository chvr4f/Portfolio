...
export interface SpecularButtonProps {
  ...
  /**
   * LOCAL ADDITION. Upstream renders a hardcoded <button>. Given an `href`
   * this renders an <a> instead, so navigations keep link semantics —
   * cmd/middle-click to open in a new tab, "copy link address", and a screen
   * reader announcing "link" rather than "button".
   */
  href?: string;
  target?: string;
  rel?: string;
...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    // LOCAL ADDITION. ogl throws outright when a GL context can't be created,
    // and an uncaught throw in an effect unmounts the tree — which would take
    // a primary call-to-action off the page. The button markup is independent
    // of the canvas, so swallowing this just costs the shine.
    let renderer;
    try {
