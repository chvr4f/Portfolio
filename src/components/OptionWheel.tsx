...
export interface OptionWheelProps {
  ...
  /* ---- LOCAL ADDITIONS (see README) ---------------------------------------
     Upstream is a self-contained picker: it owns its index, and `defaultSelected`
     only seeds state (it is absent from the sync effect's deps, so it is inert
     after mount). Driving the wheel from page scroll needs a controlled value,
     and a fixed nav must not eat the page's scroll. */

  /** Controlled, fractional position. Supplied, page scroll drives the wheel. */
  position?: number;
  /**
   * Fired only for deliberate input — a click on an option or an arrow key.
   * `onChange` also fires when `position` moves the wheel, so wiring navigation
   * to it would feed the scroll straight back into itself.
   */
  onUserSelect?: (index: number) => void;
  /**
   * Upstream binds a non-passive `wheel` listener that calls `preventDefault`,
   * and sets `touch-action: none`. In a picker that is correct; in a fixed rail
   * it means the page stops scrolling whenever the cursor crosses the nav, and
   * never scrolls at all over it on a touchscreen.
   */
...
const OptionWheel = ({
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      const cfg = cfgRef.current;
      onUserSelectRef.current?.(((next % cfg.count) + cfg.count) % cfg.count);
    },
    [applyTarget]
  );
  ...
  // Controlled mode. Feeding the fractional value straight into the existing
  // target lets the component's own easing and curve do the work, so a
  // scroll-driven wheel moves exactly like a dragged one.
  useEffect(() => {
  ...
  return (
    <div
      ...
      className={`relative h-full w-full select-none overflow-hidden outline-none ${
        captureScroll ? '[touch-action:none]' : ''
      } ${draggable ? (isDragging ? 'cursor-grabbing' : 'cursor-grab') : ''}${className ? ` ${className}` : ''}`}
      style={
        {
      ...
      {items.map((label, index) => (
        <div
          ...
          onClick={() => {
            ...
            if (!dragMovedRef.current) onUserSelectRef.current?.(index);
          }}
        >
          {label}
        </div>
